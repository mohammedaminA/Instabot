...
notnow1 = webdriver.find_element_by_css_selector('#react-root > section > main > div > div > div > div > button')
notnow1.click()
sleep(3)
    ...
    try:
        for x in range(1, 200):
            username = webdriver.find_element_by_xpath(
                '//*[@id="react-root"]/section/main/div/div[1]/article/header/div[2]/div[1]/div[1]/span/a').text

            if username not in prev_user_list:
                # If we already follow, do not unfollow
                if webdriver.find_element_by_xpath(
                        '//*[@id="react-root"]/section/main/div/div[1]/article/header/div[2]/div[1]/div[2]/button').text == 'Follow':
                    ...
                    if comm_prob > 7:
                        ...
                        if (comm_prob < 7):
                            comment_box.send_keys('I really Love it!')
                            sleep(1)
                        elif (comm_prob > 6) and (comm_prob < 9):
                            comment_box.send_keys('They really went all out :)')
                            sleep(1)
                        elif comm_prob == 9:
                            comment_box.send_keys('You got good taste on God')
                            sleep(1)
                        elif comm_prob == 10:
                            comment_box.send_keys('Unbelievable! :)')
                            sleep(1)
